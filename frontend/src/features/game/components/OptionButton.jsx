...
export default function OptionButton({ value, onClick, selected, disabled }) {
  return (
    <button onClick={() => !disabled && onClick(value)} style={{
      ...
      fontSize: 'clamp(1rem, 3vw, 1.3rem)', letterSpacing: '0.1em',
      ...
      onTouchStart={e => { if (!selected && !disabled) e.currentTarget.style.borderColor = '#00e5ff'; }}
      onTouchEnd={e => { if (!selected) e.currentTarget.style.borderColor = '#1a2d44'; }}
    >{value}</button>
  );
}